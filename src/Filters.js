...
const Filters = ({ filters, removeFilter }) => {
  return (
    <div className="flex p-4 -mt-10 mx-auto w-11/12 lg:w-9/12 max-w-screen-xl bg-white shadow-xl rounded-md text-sm font-spartan">
      {filters.length > 0 &&
        filters.map((filter) => (
          <span className="inline-flex bg-light-gray-cyan-filters text-desaturated-dark-cyan font-bold rounded">
      ...
      <button className="ml-auto font-bold underline text-desaturated-dark-cyan">
        Clear
      </button>
    </div>
  );
